...
forward_selection <- function(set.vec, grad.set, max.features=30){
  input.vec <- c()
  path.dt.list <- list()
  ...
  while(length(input.vec) < max.features){
    ...
    min.dev <- some.dev[which.min(Deviance)]
    set.err <- data.table(
      set=set.vec,
...
for(seed in seed.vec){
  ...
  train.dt <- three.path.dt[set != "test"]
  ...
  path.wide <- data.table::dcast(
    three.path.dt, model.size ~ set, value.var="loss")
  sel.dt <- penaltyLearning::modelSelection(
    three.path.dt[set=="subtrain"], "loss", "model.size")
  sel.some <- data.table(sel.dt)[, .(
    model.size, min.log.lambda, max.log.lambda, min.lambda, max.lambda
  )]
  sel.train <- train.dt[sel.some, on="model.size"]
  ggplot()+
    geom_segment(aes(
      min.log.lambda, loss,
      xend=max.log.lambda, yend=loss),
      data=sel.train)+
    facet_grid(set ~ .)
  best.valid <- sel.train[set=="validation"][which.min(loss)]
  best.log.lambda <- best.valid[, min.log.lambda+max.log.lambda]/2
  ## re-fit on train set? compare with subtrain only
  two.set.vec <- ifelse(three.set.vec=="test", "test", "train")
  two.path.dt <- forward_selection(two.set.vec, "train")
  two.sel.dt <- data.table(penaltyLearning::modelSelection(
    two.path.dt[set=="train"], "loss", "model.size"))
  selected.sizes <- rbind(two.sel.dt[
    min.log.lambda < best.log.lambda & best.log.lambda < max.log.lambda,
    .(model="train.penalty", model.size)],
    best.valid[, .(model="train.size", model.size)])
  best.valid.size <- best.valid[, .(model.size)]
  is.train <- two.set.vec=="train"
  gfit <- glmnet::cv.glmnet(X[is.train,], y[is.train], family="binomial")
  b <- coef(gfit)[-1,]
  g.pred <- predict(gfit, X[!is.train,])  
  this.test <- rbind(
    two.path.dt[set=="test"][selected.sizes, on="model.size"],
    data.table(
      three.path.dt[set=="test"][best.valid.size, on="model.size"],
      model="subtrain.size"),
    data.table(
      model.size=sum(b!=0),
      set="test",
      loss=NA,
      error.percent=100*mean(ifelse(g.pred>0, 1, 0) != y[!is.train]),
      model="train.glmnet"))    
  test.err.dt.list[[paste(seed)]] <- data.table(seed, this.test)
  two.dt.list[[paste(seed)]] <- data.table(seed, two.path.dt)
  three.dt.list[[paste(seed)]] <- data.table(seed, three.path.dt)
}

(test.err.dt <- do.call(rbind, test.err.dt.list))
(two.dt <- do.call(rbind, two.dt.list))
(three.dt <- do.call(rbind, three.dt.list))
